(ns lotus.utils.autocompleter.responsive.core
  (:require-macros
...
(defn selector
  ([in list selection-data]
     (.log js/console "inside selector")
     (selector in list selection-data selection-data))
  ([in list visual-data selection-data]
     (let [out (chan)]
                 ...
                 (do
                   ...
                   (if (number? highlighted)
                     (do
                       ...
                       (>! out [:select [ (nth visual-data highlighted)
                                          (nth selection-data highlighted)]]))
                     (>! out [:select [highlighted highlighted]]))
...
(defn key-events [prevent-default?]
  (let [prevent-fn #(when @prevent-default?
                      (.preventDefault %)
                      (.stopPropagation %))]
   (->> (r/listen js/document :keydown prevent-fn)
